...
const PRODUCTS = [
    ...
    { id: 8, name: "Kölsch", size: "0,2l", price: 2.30, category: "Bier", fetchFromBelow: false },
...
function addToOrder(productId) {
    ...
    const existingOrder = table.orders.find(o => o.productId === productId && !o.comment && !o.isPaid);
        ...
        return `
            <div class="order-item">
                <div class="order-header">
                    <div class="order-name">${escapeHtml(product.name)}${product.size ? ` (${product.size})` : ''}</div>
                ...
                <div class="order-actions">
                    ...
                    <button class="btn btn-small btn-success" onclick="markPaid(${table.id}, ${order.id})">✓ Bezahlt</button>
                </div>
            </div>
        `;
    }).join('');
...
function markPaid(tableId, orderId) {
    const table = state.tables.find(t => t.id === tableId);
    const order = table.orders.find(o => o.id === orderId);
    order.isPaid = true;
    saveState();
...
function renderFetchList() {
    ...
    // Collect all fetch-from-below items across all tables
    const fetchItems = [];
            ...
            ${group.items.map(item => `
                <div class="fetch-item-row">
                    <div class="fetch-item-info">
                        <div class="fetch-item-name">${escapeHtml(item.product.name)}</div>
...
function updateBadges() {
    ...
    state.tables.forEach(table => {
        table.orders.filter(o => !o.isPaid).forEach(order => {
            const product = PRODUCTS.find(p => p.id === order.productId);
            if (product && product.fetchFromBelow) {
                fetchCount += order.quantity;
            }
        });
    });
    
    const fetchBadge = document.getElementById('fetchBadge');
    fetchBadge.textContent = fetchCount;
    fetchBadge.classList.toggle('visible', fetchCount > 0);
}
